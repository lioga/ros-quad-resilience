...
if (set_mode_client.call(offb_set_mode) && offb_set_mode.response.mode_sent)
  ...
  while(current_state.mode != "GUIDED_NOGPS")
  {
    ros::spinOnce();
    ros::Duration(0.01).sleep();
  }
...
/*
  //disarming
  //ros::ServiceClient disarming_client_i = nh.serviceClient<mavros_msgs::CommandBool>("mavros/cmd/arming");
  mavros_msgs::CommandBool srv_disarm_i;
  srv_disarm_i.request.value = false;
  if (arming_client_i.call(srv_disarm_i) && srv_disarm_i.response.success)
    ROS_INFO("DISARM sent %d", srv_disarm_i.response.success);
  else
  {
